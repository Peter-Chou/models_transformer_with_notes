  ...
  def call(self, x, padding=None):
    ...
    Args:
      ...
      padding: (optional) If set, the padding values are temporarily removed
        ...
        shape: [batch_size, length], 0 is non-padding value, 1 is padding value
        using padding ensures the <PAD> vector is always zero vector
    ...
    # shape[-1] from hidden_size to filter_size with relu activation
    output = self.filter_dense_layer(x)
    if self.train:
      output = tf.nn.dropout(output, 1.0 - self.relu_dropout)

    # shape[-1] from filter_size to hidden_size
