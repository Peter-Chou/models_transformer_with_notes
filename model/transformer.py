...
class Transformer(object):
  ...
  sequence (or a batch of sequences). 
  The encoder produces a continous representation
  the decoder uses the encoder output to generate probabilities
  ...
  def __call__(self, inputs, targets=None):
    ...
    with tf.variable_scope("Transformer", initializer=initializer):
      ...
      # shape: (batch_size, 1, 1, input_length)
      # *The tensor is zero at non-padding locations, and -1e9 at padding locations.
  ...
  def encode(self, inputs, attention_bias):
    ...
    with tf.name_scope("encode"):
      ...
      # applying dropout.
      # shape: (batch_size, input_length, hidden_size)
      embedded_inputs = self.embedding_softmax_layer(inputs)
      # shape: (batch_size, input_length)
      # non-padding value: 0 , padding value: 1
...
class EncoderStack(tf.layers.Layer):
  ...
  def call(self, encoder_inputs, attention_bias, inputs_padding):
    ...
    Args:
      ...
      inputs_padding: [batch_size, length], 1 for padding, 0 for non-padding
...
class DecoderStack(tf.layers.Layer):
  ...
  def call(self, decoder_inputs, encoder_outputs, decoder_self_attention_bias,
    ...
    for n, layer in enumerate(self.layers):
      ...
      # cache里保存了decoder的每个stack里 masked self attention layer 该时刻之前的
      # 所有时刻的(key vector, value vector), 其中 query == key
      # shapes of keys / values: (batch_size, input_length, params["hidden_size"])
      layer_cache = cache[layer_name] if cache is not None else None
      with tf.variable_scope(layer_name):
        with tf.variable_scope("self_attention"):
          # ! decoder_self_attention_bias for masked attention layer in decoder
          decoder_inputs = self_attention_layer(
              decoder_inputs, decoder_self_attention_bias, cache=layer_cache)
        with tf.variable_scope("encdec_attention"):
          # ! attention_bias for encoder-decoder attention layer in decoder
